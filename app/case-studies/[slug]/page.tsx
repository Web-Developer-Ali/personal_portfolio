...
const caseStudies: Record<string, any> = {
  'scales-flow': {
    ...
    heroDescription: 'Most 10–30 person marketing agencies track deals in Google Sheets, send updates on WhatsApp, and give their admin zero real-time visibility. Scales Flow was built to fix exactly that.',
    meta: [
    ...
    pipelineStages: [
      { label: 'Prospect', style: 'bg-blue-500/15 text-blue-400 border-blue-500/30' },
      ...
      { label: 'Negotiation', style: 'bg-white/5 text-white/50 border-white/10' },
      { label: 'Closed won', style: 'bg-emerald-500/15 text-emerald-400 border-emerald-500/30' },
    ],
    capabilities: [
      {
  ...
  'jobord': {
    ...
    heroDescription: 'JobOrd rethinks the job search experience. Instead of applying to hundreds of jobs and hearing nothing back, job seekers create a profile once, employers find them when there\'s a real fit, and the hiring process moves at human speed.',
    meta: [
      ['Role', 'Full-stack developer'],
      ['Stack', 'Next.js + PostgreSQL'],
  ...
  'workspan': {
    ...
    heroDescription: 'WorkSpan brings together the best parts of project management, communication, and documentation into one unified platform. Teams spend less time context-switching between tools and more time doing focused work.',
    meta: [
      ['Role', 'Full-stack developer'],
      ['Stack', 'Next.js + PostgreSQL'],
    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-6 py-16 space-y-24">
        ...
        <nav className="flex items-center gap-2 text-sm text-white/30">
          ...
          <span>/</span>
          <span className="text-white/60">{study.title.split(' — ')[1] || study.title}</span>
        </nav>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-8 text-sm text-white/40">
            {study.meta.map(([label, value]: [string, React.ReactNode]) => (
              <div key={label}>
        ...
        <section>
          ...
          <h2 className="text-2xl font-semibold text-white mb-3">
            {slug === 'scales-flow' ? 'Small agencies are running serious revenue operations on tools built for something else' : 
             slug === 'skillsphere' ? 'Online education lacks the structure and tools learners need' :
             slug === 'jobord' ? 'The hiring process is broken for both sides' :
             'Distributed teams waste time context-switching between tools'}
          </h2>
          <p className="text-white/50 mb-10 max-w-2xl leading-relaxed">
            {slug === 'scales-flow' ? 'Google Sheets, WhatsApp threads, Notion boards — these were never CRM tools. For a 10–30 person agency with multiple sales reps and a manager, they create blind spots at every level of the team.' :
             slug === 'skillsphere' ? 'Learning platforms often feel fragmented. Content is scattered across multiple services, progress tracking is manual, and video delivery is unreliable.' :
             slug === 'jobord' ? 'Job seekers apply to positions and disappear into a black hole. Employers get overwhelmed with unqualified applications. Neither side wins.' :
        ...
        <section>
          ...
          <h2 className="text-2xl font-semibold text-white mb-3">
            {slug === 'scales-flow' ? 'Three roles. One system. Everyone sees what they need.' :
             slug === 'skillsphere' ? 'One platform. Complete learning experience.' :
             slug === 'jobord' ? 'A job platform built around real relationships.' :
             'One unified workspace for all team work.'}
          </h2>
          <p className="text-white/50 mb-10 max-w-2xl leading-relaxed">
            {slug === 'scales-flow' ? 'Instead of building another generic CRM, Scales Flow is designed around how agency sales teams actually work. Each role gets a dashboard scoped to their responsibilities — nothing more, nothing less.' :
             slug === 'skillsphere' ? 'SkillSphere brings all learning materials, progress tracking, and communication into one place. Students learn better. Instructors manage easier.' :
             slug === 'jobord' ? 'JobOrd creates a direct connection between candidates and employers. No endless applications. No automated rejections. Just meaningful matches and real conversations.' :
          ...
          <div className="grid sm:grid-cols-3 gap-4 mb-12">
            {study.roles.map((r: any) => (
              ...
              >
                <span className={`text-[10px] font-semibold tracking-widest uppercase ${r.color} block mb-3`}>
                  {r.label}
                </span>
                <h3 className="font-semibold text-white mb-3 text-sm">{r.title}</h3>
                <ul className="space-y-1.5">
                  {r.features.map((f: string) => (
                    <li key={f} className={`text-sm text-white/45 flex items-start gap-2`}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {study.pipelineStages.map((s: any, i: number) => (
                  <div key={s.label} className="flex items-center gap-2">
                    <span className={`px-3 py-1.5 rounded-md text-xs font-medium border ${s.style}`}>
                      {s.label}
                    </span>
        ...
        <section>
          ...
          <h2 className="text-2xl font-semibold text-white mb-10">
            {slug === 'scales-flow' ? 'The capabilities a growing agency needs' :
             slug === 'skillsphere' ? 'Creating the ideal learning environment' :
             slug === 'jobord' ? 'A better hiring experience for everyone' :
        ...
        <section className="border border-emerald-500/25 bg-emerald-500/[0.04] rounded-2xl p-8">
          ...
          <p className="text-white/50 mb-6 max-w-lg leading-relaxed">
            I&apos;d love to discuss how these technologies and approaches could apply to your project. Let&apos;s talk about what&apos;s possible.
          </p>
